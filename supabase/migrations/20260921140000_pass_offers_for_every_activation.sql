-- Day 5 and Day 7 offers go to every activated pass. The Meta lead form already
-- carries the guest's agreement, and the website claim now discloses these emails.
-- Joined, do not contact, unsubscribe, staff pause, and the send window still stop them.
...
-- Release offers held only for consent; the next planner run re-evaluates every other stop.
update revive_private.pass_followup_jobs set blocked_reason=null where status='pending' and blocked_reason='Marketing consent required';

-- The website checkbox and the staff consent checkbox are gone; historical consent columns stay for the record.
drop function public.revive_pass_marketing_consent(text,text);
drop function revive_private.pass_marketing_consent(text,text);
