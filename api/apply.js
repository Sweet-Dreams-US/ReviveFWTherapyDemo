...
// 1) Cloudflare Turnstile bot check, fail closed.
// 2) Stores the application in Supabase via a SECURITY DEFINER RPC (validates the job is open).
// 3) Best-effort staff notification. HR data is never sent to the gym advertising dataset.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!TURNSTILE_SECRET) return res.status(503).json({ error: 'Security check is temporarily unavailable.' });
    if (TURNSTILE_SECRET) {
      ...
      try {
        ...
        const vr = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
          method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: params, signal: AbortSignal.timeout(10000),
        });
        const vj = await vr.json().catch(() => ({ success: false }));
        if (!vr.ok || vj.success !== true || vj.action !== 'job_application' || !['revivefw.com', 'www.revivefw.com'].includes(vj.hostname)) { res.status(400).json({ error: 'Bot verification failed. Please try again.' }); return; }
...
async function notifyEmail(row, req) {
  ...
  const rows = [['Applicant', name], ['Email', row.email], ['Phone', row.phone || 'Not provided'], ['Role', row.jobTitle || 'Not provided']];
