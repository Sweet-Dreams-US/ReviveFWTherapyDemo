...
test('authored email subjects and rendered copy have no dashes', () => {
  for (const template of [passEmail, experienceEmail]) {
    const email = template('cole@sweetdreams.us');
    assert.doesNotMatch(email.subject, /[-\u2010-\u2015]/);
    assert.doesNotMatch(clean(email.text), /[-\u2010-\u2015]/);
    assert.doesNotMatch(clean(email.html.replace(/<[^>]*>/g, '')), /[-\u2010-\u2015]/);
  }
  const email=experienceEmail('cole@sweetdreams.us');
  assert.match(email.html,/How did/);assert.match(email.html,/Just reply to this email/);
  assert.match(email.html,/mailto:info@revivefw.com/);
...
test('experience preview records a test without changing redemption and rejects duplicate test sends', async () => {
  ...
  const lead={id:'test-id',email:'cole@sweetdreams.us',notes:'Existing notes',feedback:'Recovery',version:4,do_not_contact:false};
  let sends=0;
  global.fetch=async(url,options)=>{
    ...
    else if(url==='https://api.resend.com/emails'){sends++;assert.equal(options.headers['Idempotency-Key'],'experience-preview-v1-test-id');data={id:'provider-test'};}
