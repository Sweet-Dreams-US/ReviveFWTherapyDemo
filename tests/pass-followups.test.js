...
test('Followup templates: scoped opt-out, private feedback, approved offer, no dashes in copy',()=>isolated(async()=>{
  ...
  for(const stage of ['experience','day5','day7','day10','day13']){
    const mail=template({stage,lead_id:id,email:'guest@example.com',activated_at:'2026-09-12T16:00:00Z'});
    ...
    if(stage==='experience'){assert.match(mail.html,/pass-feedback#token=/);assert.doesNotMatch(mail.text,/help.*plan.*visit|just reply/i);}
      ...
      // Reopens the approved PT bonus with its own same day deadline; no rates or discounts.
      assert.match(mail.text,/Kings Nutrition/);assert.match(mail.text,/Elite: 2 free PT sessions/);assert.match(mail.text,/For today only/);
      assert.match(mail.text,/This offer ends: Thursday, September 24, 2026.*11:00 PM Eastern/);
      assert.doesNotMatch(mail.text,/Your pass and joining bonus end|per month|six month/i);}
